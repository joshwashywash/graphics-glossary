---
description: An explanation of the concept of "Level Of Detail" and how it can be achieved in three.js.
title: Level Of Detail
---

import App from "@components/techniques/level-of-detail/app.svelte";
import Tabs from "@components/tabs.astro";
import UnderConstructionAside from "@components/underConstructionAside.astro";
import tabItems from "@components/techniques/level-of-detail/tabItems";

Level of Detail or LOD is a performance-improving technique where the detail or complexity of an object's geometry decreases as relative to how far it is from the camera. Objects that are further away are less detailed because finer details will probably go unnoticed to the viewer.

<App client:visible />
<Tabs items={tabItems} />

The performance boost comes from the fact that the renderer spends less time rendering lower quality versions of the object.

## Usage

Conveniently three.js has built-in support for LOD.

<UnderConstructionAside>
	todo: basic usage example code with explanation
</UnderConstructionAside>

## References

- [three.js LOD docs](https://threejs.org/docs/#api/en/objects/LOD)
