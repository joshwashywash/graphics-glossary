...
ThreeJS's ShadowMesh addon is a very performant alternative to shadow mapping. It makes use of the stencil buffer and a locally computed matrix to simulate very simple shadows.
...
The light source does not need to be added to the scene and since the light's position is the only things that is considered, it doesn't even need to be a light object.
...
```ts
const lightPosition = new Vector3(5, 5, 5);
const light4D = new Vector4(...lightPosition, 0.1);

shadowMesh.update(plane, light4D);
```

This is because the shadow mesh can only be _casted_ on planes and the only information that is needed from the light source when calculating the local matrix is the light source's position - the assumption being that the light source is pointed toward the plane.
