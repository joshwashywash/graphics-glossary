...
Surfaces have more [specular reflection](https://en.wikipedia.org/wiki/Specular_reflection) when viewed at near-grazing angles. This phenomenon is often called the fresnel effect. It is typically used as one of the contributing factors for the specular reflection term in physically-based renderinga . The fresnel factor is often approximated using [Schlick's approximation](https://en.wikipedia.org/wiki/Schlick%27s_approximation).
...
This fragment shader is an even simpler than Schlick's approximation. It uses the dot product of the normal vector of the surface and the direction towards the camera. The fresnel factor will be greatest when the surface normal is perpendicular to the direction vector. This causes an outline effect as edges of the surface curve away from the camera's view.

The effect works best with curvy surfaces because the normals are more continuous over the surface's edges.
