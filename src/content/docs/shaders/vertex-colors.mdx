...
import ButterfreeApp from "@components/examples/shaders/vertex-colors/butterfree/app.svelte";
import SimpleApp from "@components/examples/shaders/vertex-colors/simple/app.svelte";
...
<ButterfreeApp client:only />

### Representing Data Through Coloring

You can use vertex colors to represent data that can be expressed through a geometry's vertices. This really only works if the data is **per vertex** and is can be interpreted correctly when interpolated. For example, mapping height of a vertex to a color might consist of representing _higher_ vertices to more vibrant colors.
